...
int main(int argc, char **argv) {
    if (argc != 5) {
        printf("please run this program by the following parameter: kdtree_high sample_number filePath point_number\n");
        return 0;
    }
    ...
    fin.close();
    const int point_data_size = point_data.size() < point_number ? point_data.size() : point_number;
